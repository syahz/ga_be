...
describe('Unit API (/api/admin/units)', () => {
  ...
  // Bersihkan data unit yang dibuat khusus untuk tes setelah setiap tes selesai
  afterEach(async () => {
  ...
  describe('POST /api/admin/units', () => {
    it('should create a new unit successfully', async () => {
      ...
      logger.debug('POST /api/admin/units (success): %s', JSON.stringify(response.body, null, 2))
    ...
    it('should fail with 409 if unit code already exists', async () => {
      ...
      logger.debug('POST /api/admin/units (duplicate code): %s', JSON.stringify(response.body, null, 2))
    ...
    it('should fail with 401 if not authenticated', async () => {
      ...
      logger.debug('POST /api/admin/units (unauthenticated): %s', JSON.stringify(response.body, null, 2))
  ...
  describe('GET /api/admin/units', () => {
    ...
    it('should get a paginated list of units', async () => {
      ...
      logger.debug('GET /api/admin/units (paginated): %s', JSON.stringify(response.body, null, 2))
      expect(response.status).toBe(200)
      expect(response.body.data.units).toHaveLength(2)
      expect(response.body.data.pagination.total_data).toBe(3)
    })

    it('should get a list of units with a search query', async () => {
      const response = await supertest(web).get('/api/admin/units?search=Unit A').set('Authorization', `Bearer ${adminToken}`)

      logger.debug('GET /api/admin/units (search): %s', JSON.stringify(response.body, null, 2))
      expect(response.status).toBe(200)
  ...
  describe('GET /api/admin/units/all', () => {
    it('should get all units without pagination', async () => {
      ...
      logger.debug('GET /api/admin/units/all: %s', JSON.stringify(response.body, null, 2))
      expect(response.status).toBe(200)
  ...
  describe('GET /api/admin/units/:unitId', () => {
    it('should get a unit by its ID', async () => {
      ...
      logger.debug('GET /api/admin/units/:unitId (success): %s', JSON.stringify(response.body, null, 2))
      expect(response.status).toBe(200)
    ...
    it('should fail with 404 if unit ID is not found', async () => {
      ...
      logger.debug('GET /api/admin/units/:unitId (not found): %s', JSON.stringify(response.body, null, 2))
  ...
  describe('PUT /api/admin/units/:unitId', () => {
    it('should update a unit successfully', async () => {
      ...
      const response = await supertest(web)
        ...
        .send({ code: 'TEST-NEW-CODE', name: 'TEST-UnitTerupdate' })

      logger.debug('PUT /api/admin/units/:unitId (success): %s', JSON.stringify(response.body, null, 2))
      expect(response.status).toBe(200)
      expect(response.body.data.name).toBe('TEST-UnitTerupdate')
      expect(response.body.data.code).toBe('TEST-NEW-CODE')
    })
    ...
    it('should fail with 409 if new name already exists', async () => {
      ...
      logger.debug('PUT /api/admin/units/:unitId (duplicate name): %s', JSON.stringify(response.body, null, 2))
      expect(response.status).toBe(409)
    })
  })
  ...
  describe('DELETE /api/admin/units/:unitId', () => {
    it('should delete a unit successfully if not in use', async () => {
      ...
      logger.debug('DELETE /api/admin/units/:unitId (success): %s', JSON.stringify(response.body, null, 2))
      expect(response.status).toBe(200)
    ...
    it('should fail with 400 if unit is still in use by a user', async () => {
      ...
      logger.debug('DELETE /api/admin/units/:unitId (in use): %s', JSON.stringify(response.body, null, 2))
